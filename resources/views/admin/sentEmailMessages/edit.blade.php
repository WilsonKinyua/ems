...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sentEmailMessage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sent-email-messages.update", [$sentEmailMessage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="sent_to_name">{{ trans('cruds.sentEmailMessage.fields.sent_to_name') }}</label>
                <input class="form-control {{ $errors->has('sent_to_name') ? 'is-invalid' : '' }}" type="text" name="sent_to_name" id="sent_to_name" value="{{ old('sent_to_name', $sentEmailMessage->sent_to_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sent_to_email') ? 'is-invalid' : '' }}" type="email" name="sent_to_email" id="sent_to_email" value="{{ old('sent_to_email', $sentEmailMessage->sent_to_email) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subject') ? 'is-invalid' : '' }}" type="text" name="subject" id="subject" value="{{ old('subject', $sentEmailMessage->subject) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message" required>{{ old('message', $sentEmailMessage->message) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('token') ? 'is-invalid' : '' }}" name="token" id="token">{{ old('token', $sentEmailMessage->token) }}</textarea>
                @if($errors->has('token'))
                    <div class="invalid-feedback">
                        {{ $errors->first('token') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('read') ? 'is-invalid' : '' }}" type="number" name="read" id="read" value="{{ old('read', $sentEmailMessage->read) }}" step="1" required>
                @if($errors->has('read'))
                    <div class="invalid-feedback">
                        {{ $errors->first('read') }}
                    </div>
